#!/usr/bin/env bash
# Regenerate the keccak Lean extraction via hax's aeneas-based `lean` backend.
#
...
#      patches (keccak SIMD gating + the p3-field / p3-symmetric hax gates that
...
#      P3Keccak/Extraction/{Types,Funs,*_Template}.lean + llbc/ (gitignored).
#      aeneas reports 2 `Unreachable` errors (the two derive(Debug) impls) and
...
#   lakefile.toml lean-toolchain P3Keccak.lean
#   P3Keccak/Extraction/TypesExternal.lean P3Keccak/Extraction/FunsExternal.lean
...
SRC_PATCHES=(
  "keccak/proofs/rust-patch/patches/p3_keccak.src.patch"
  "field/proofs/rust-patch/patches/p3_field.src.patch"
  "symmetric/proofs/rust-patch/patches/p3_symmetric.src.patch"
)
REVERT_PATHS=(keccak/src keccak/Cargo.toml field/src symmetric/src)
...
isolated="${HAX_ISOLATED_TARGET:-/tmp/hax-extract/target-keccak-aeneas}"
rm -rf "$isolated"
( cd "$CRATE_ROOT" && \
  ...
  cargo hax into lean ) \
  || echo "WARNING: cargo hax exited non-zero (expected: 2 aeneas Unreachable errors on derive(Debug)); judging from lake build below." >&2
...
cp "$SCRIPT_DIR/P3Keccak/Extraction/Funs.lean" "$SCRIPT_DIR/patches/Funs.pristine.lean"
