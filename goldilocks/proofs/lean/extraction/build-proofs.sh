#!/usr/bin/env bash
# Regenerate the goldilocks Lean extraction from Rust source.
#
# goldilocks is the prime field F_p, p = 2^64 - 2^32 + 1 (64-bit). It carries
# hand-written SIMD packings (aarch64 NEON / x86_64 AVX2,AVX512 / wasm32 simd128)
# that hax cannot model. We drop the SIMD modules *before* extraction with a
# small, behaviour-preserving Rust source patch that gates them on `not(hax)` and
# forces the portable `Packing = Self` path under `--cfg hax`. A normal
# `cargo build` is unaffected. (goldilocks has no DFT module.)
#
...
#   4. apply patches/p3_goldilocks.patch (the Lean patch).
...
# p3_goldilocks.lean until it compiles, then run patches/update-patch.sh.
...
TARGET="$EXTRACT_DIR/p3_goldilocks.lean"
PRISTINE="$PATCH_DIR/pristine.snapshot.lean"
PATCH_FILE="$PATCH_DIR/p3_goldilocks.patch"
SRC_PATCH="$CRATE_ROOT/proofs/rust-patch/patches/p3_goldilocks.src.patch"

# ---- 0. source patch: clean-tree guard + guaranteed revert + apply ----------
SRC_REL="goldilocks/src"
if [ -f "$SRC_PATCH" ]; then
...
isolated="${HAX_ISOLATED_TARGET:-/tmp/hax-extract/target-goldilocks-lean}"
rm -rf "$isolated"
( cd "$CRATE_ROOT" && \
...
# goldilocks's qm31.rs hits HAX0001 (#1923, assoc-type equality constraints) on
# the BasedVectorSpace `from_basis_coefficients_*` methods, so hax exits non-zero
# but still writes a usable p3_goldilocks.lean. Do NOT abort here (see koala note);
