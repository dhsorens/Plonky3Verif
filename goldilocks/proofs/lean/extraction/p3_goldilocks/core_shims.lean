...
-- `core::cmp::Ord::min` (proof-lib provides cmp but not the min helper).
namespace core_models.cmp
def Ord.min (T : Type) (a : T) (_b : T) : RustM T := pure a
end core_models.cmp

namespace core_models.num
def Impl_2.overflowing_add (x : i32) (_y : i32) : RustM (i32 × Bool) := pure (x, false)
end core_models.num

-- `Iterator::{map, sum}` (the proof-lib lacks these as callable projections).
-- Used in trait default bodies (generic linear-layer plumbing); stubbed.
namespace core_models.iter.traits.iterator
def Iterator.map (S B F : Type) (_self : S) (_f : F) :
  RustM (core_models.iter.adapters.map.Map S F) := sorry
def Iterator.sum (S B : Type) (_self : S) : RustM B := sorry
end core_models.iter.traits.iterator

namespace core_models.hash
def Hasher.write_u32 (H : Type) (h : H) (_x : u32) : RustM H := pure h
end core_models.hash

namespace core_models.num
def Impl_9.overflowing_add (x : u64) (_y : u64) : RustM (u64 × Bool) := pure (x, false)
def Impl_9.wrapping_add_signed (x : u64) (_y : i64) : RustM u64 := pure x
def Impl_3.MIN : i64 := sorry
end core_models.num

namespace core_models.hash
def Hasher.write_u64 (H : Type) (h : H) (_x : u64) : RustM H := pure h
end core_models.hash

@[reducible] instance : core_models.fmt.Display.AssociatedTypes u64 where
@[spec] instance : core_models.fmt.Display u64 where fmt := sorry

namespace core_models.num
def Impl_9.wrapping_neg (x : u64) : RustM u64 := pure x
end core_models.num

@[spec] instance : Cast Bool u64 where cast _ := pure sorry

@[reducible] instance : core_models.convert.From.AssociatedTypes u64 Bool where
@[spec] instance : core_models.convert.From u64 Bool where _from := sorry
